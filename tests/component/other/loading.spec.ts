...
describe('加载组件', function () {
  ...
  describe('参数测试', () => {
    ...
    test('设置sideLength为200', () => {
      ...
      expect(wrapper.find('.m-loading-item').element.getAttribute('style'))
        .toBe('height: 200px; width: 200px;');
      wrapper.unmount();
    });
  ...
  describe('动画循环', () => {
    ...
    const getItemClass = (wrapper: VueWrapper) => wrapper.findAll('.m-loading-item').map(item => item.element.className
      .replace('m-loading-item ', ''));
