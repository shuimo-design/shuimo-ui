...
export const getEsConfig = (pkg = 'lib', pkgName = 'shuimo-ui') => {
  const { plugins: basePlugins, buildConfig, getPath, baseRoot, fileName } = getCommonConfig(pkg, pkgName);
  const outputRoot = path.resolve(__dirname, `../../../${pkg}/dist`);

  // init components records
  // 扫描${pkg}/components下的所有文件
  ...
  class EsConfig {
    ...
    cssBuildPlugin() {
      ...
      return {
        ...
        transform(code, id) {
          if (!(id.includes(`${pkg}/components`) && id.endsWith('.css'))) {
            return;
          }
