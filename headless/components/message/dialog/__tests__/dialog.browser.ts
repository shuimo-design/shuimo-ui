/**
 * @description headless Dialog 浏览器测试
 * @author 阿怪
 * @date 2026/2/25
 * @version v1.0.0
 *
...
// 包装组件：通过模板 ref 访问 expose 的方法
function makeWrapper(dialogProps: Record<string, unknown> = {}, slotContent = '弹窗内容') {
  return defineComponent({
    setup() {
      const dialogRef = ref<{ open: () => void; close: () => void; toggle: () => void } | null>(null);
      return { dialogRef };
    },
    render() {
      return h(MDialog, {
...
describe('MDialog browser', () => {
  ...
  it('点击 active 区域触发 toggle 打开弹窗', async () => {
    const { container } = render(MDialog, {
      props: { visible: false },
      slots: {
        default: () => '弹窗内容',
  ...
  it('遮罩层点击后弹窗关闭（默认 clickClose=true）', async () => {
    const onUpdate = vi.fn();
    render(MDialog, {
      props: { visible: true, 'onUpdate:visible': onUpdate },
      slots: { default: () => '弹窗内容' },
