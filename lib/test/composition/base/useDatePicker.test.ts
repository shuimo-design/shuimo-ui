...
describe('useDatePicker core test', function () {
  ...
  describe('date calculation', function () {
    ...
    describe('value to displayValue', function () {
      ...
      it('empty value will be placeholder', () => {
        const { displayValue, spanClass } = useDatePicker({
          props: baseProps,
        });
    ...
    describe('func', () => {
      ...
      it('should update dateRef correctly when toPrevMonth is called', () => {
        ...
        expect(datePicker.dateRef.value).toMatchObject({ year: 2021, month: 12, day: 1 });
      });

      it('should update dateRef correctly when toNextMonth is called', () => {
        datePicker.toNextMonth();
        expect(datePicker.dateRef.value).toMatchObject({ year: 2022, month: 2, day: 1 });
      });

      it('should update dateRef correctly when toPrevYear is called', () => {
        datePicker.toPrevYear();
        expect(datePicker.dateRef.value).toMatchObject({ year: 2021, month: 1, day: 1 });
      });

      it('should update dateRef correctly when toNextYear is called', () => {
        datePicker.toNextYear();
        expect(datePicker.dateRef.value).toMatchObject({ year: 2023, month: 1, day: 1 });
      });
    });
  ...
  describe('getCalendar', () => {
    ...
    beforeEach(() => {
      const { getCalendar } = useDatePicker({ props: baseProps });
